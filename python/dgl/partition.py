...
def kahip_partition_assignment(
    ...
    if balance_edges:
        if balance_ntypes is None:
            vwgt.append(F.astype(np.add(F.asnumpy(g.in_degrees()),F.asnumpy(g.out_degrees())), F.int64))
        else:
            for ntype in uniq_ntypes:
                nids = F.asnumpy(F.nonzero_1d(balance_ntypes == ntype))
                degs = np.zeros((g.num_nodes(),), np.int64)
...
def kahip_partition(
    ...
    # Then we split the original graph into parts based on the KaHIP partitioning results.
    return partition_graph_with_halo(
        g, node_part, extra_cached_hops, reshuffle
    )[0]
